...
export class PriceFeedController {
  ...
  @ApiOperation({
    ...
    description: 'Returns historical price data for Aeternity as an array of [timestamp_ms, price] pairs.',
  })
  @ApiQuery({
    name: 'currency',
    type: 'string',
  ...
  @ApiQuery({
    ...
    required: false,
    description: 'Interval for historical data points (default: daily). Minute interval is only available for 1 day. Hourly interval is only available for 1-90 days. For days>90, only daily data is available.',
    example: 'daily',
  })
  @ApiOkResponse({
   ...
   */
  private aggregateToHourly(minuteData: Array<[number, number]>): Array<[number, number]> {
    ...
    for (const [timestamp, price] of minuteData) {
      ...
      const hourStart = Math.floor(timestamp / (1000 * 60 * 60)) * (1000 * 60 * 60);
