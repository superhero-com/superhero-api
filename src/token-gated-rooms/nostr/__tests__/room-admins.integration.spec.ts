 ...
 * against the local `groups_relay` (Task 02 harness, relay-admin keypair from
 * `settings.test.yml`). The bot nsec under test MUST be the relay admin (D7).
 *
...
describeRelay('Task 08 relay admins (integration)', () => {
  it('seeding admins puts each configured admin into 39001 (admins) and 39002 (members)', async () => {
    ...
    await writer.publish(editMetadata(gid, { name: '$ADM', isPrivate: false }));

    expect((await writer.publish(putUser(gid, adminA, 'admin'))).ok).toBe(true);
    expect((await writer.publish(putUser(gid, adminB, 'admin'))).ok).toBe(true);
  ...
  it('idempotency: re-seeding the same admin produces no duplicate in 39001', async () => {
    const writer = new RelayWriterService(
      makeConfig(RELAY_URL, RELAY_ADMIN_NSEC),
    );
    await writer.onModuleInit();
  ...
  it('converge: a 9006 set-roles=member demotes an admin no longer configured', async () => {
    const writer = new RelayWriterService(
      makeConfig(RELAY_URL, RELAY_ADMIN_NSEC),
    );
    await writer.onModuleInit();
  ...
  it('health-check: the correct relay-admin key passes; a non-admin key fails fast', async () => {
