...
describe('ProfileRewardsController', () => {
  ...
  it('is a pure read: no refresh, payout or X scan on a status read', async () => {
    // This endpoint is unauthenticated, so it must not settle, pay or scan for
    // an address the caller has not proven they own. Due rows are settled by the
    // scheduled sweep instead; the only thing a GET may do is read the status.
    const profileXPostingRewardService = {
      getRewardStatus: jest.fn().mockResolvedValue({ status: 'pending' }),
