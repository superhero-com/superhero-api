...
export class ProfileXInviteService {
  ...
  async createInvite(params: {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      try {
        ...
        if (!this.isInviteCodeUniqueConstraintError(error) || attempt === 2) {
          throw error;
        }
      }
    }
  ...
  private async sendMilestoneRewardIfEligible(
    ...
    if (!shouldSendReward || !rewardEntry || rewardEntry.status !== 'pending') {
      return;
    }
  ...
  private isInviteCodeUniqueConstraintError(error: unknown): boolean {
    ...
    return (
      ...
      (constraint.includes('code') ||
        ...
        detail.includes(' profile_x_invites ') ||
        detail.includes('profile_x_invites'))
    );
  }
