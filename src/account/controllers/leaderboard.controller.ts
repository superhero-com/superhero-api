...
export class LeaderboardController {
	...
	@ApiQuery({ name: 'window', required: false, enum: ['7d', '30d', 'all'], example: '7d' })
	@ApiQuery({ name: 'sortBy', required: false, enum: ['pnl', 'roi', 'mdd', 'aum'], example: 'pnl' })
	@ApiQuery({ name: 'sortDir', required: false, enum: ['ASC', 'DESC'], example: 'DESC' })
	@ApiQuery({ name: 'page', required: false, example: 1 })
	@ApiQuery({ name: 'limit', required: false, example: 18 })
	@ApiQuery({ name: 'points', required: false, example: 30, description: 'Approximate number of sparkline points' })
	@ApiQuery({ name: 'minAumUsd', required: false, example: 1, description: 'Exclude leaders with AUM below this USD value' })
	@ApiQuery({ name: 'maxCandidates', required: false, example: 36, description: 'Upper bound of candidate addresses to evaluate' })
	@CacheTTL(60) // cache 1 minute
	@Get()
	async getLeaderboard(
		@Query('window') window: LeaderboardWindow = '7d',
		@Query('sortBy') sortBy: LeaderboardSortBy = 'pnl',
		@Query('sortDir') sortDir: LeaderboardSortDir = (sortBy === 'mdd' ? 'ASC' : 'DESC') as LeaderboardSortDir,
		...
		@Query('maxCandidates', new DefaultValuePipe(36), ParseIntPipe) maxCandidates = 36,
	): Promise<LeaderboardResponseDto> {
		const params: GetLeadersParams = { window, sortBy, sortDir, page, limit, points, minAumUsd, maxCandidates };
