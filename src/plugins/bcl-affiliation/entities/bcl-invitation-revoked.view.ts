...
@ViewEntity({
  name: 'bcl_invitation_revoked_view',
  materialized: false,
  synchronize: true,
  expression: `
    SELECT
    ...
    WHERE contract_id = '${BCL_AFFILIATION_CONTRACT.contractAddress}'
      AND function = '${BCL_AFFILIATION_CONTRACT.FUNCTIONS.revoke_invitation_code}'
      AND data->'bcl-affiliation' IS NOT NULL
      AND jsonb_typeof(data->'bcl-affiliation'->'data'->'invitations') = 'array'
      AND data->'bcl-affiliation'->'data'->>'event_name' = 'InvitationRevoked'
  `,
})
export class BclInvitationRevoked {
  @PrimaryColumn()
  @ViewColumn()
  ...
  @ViewColumn()
  @ApiProperty({
    description: 'Event name (InvitationRevoked)',
    required: false,
    nullable: true,
  })
  event_name?: string;
