...
@ViewEntity({
  name: 'bcl_invitation_redeemed_view',
  materialized: false,
  synchronize: true,
  expression: `
    SELECT
      ...
      invitation->>'redeemer' as redeemer
    FROM txs
    ...
    WHERE contract_id = '${BCL_AFFILIATION_CONTRACT.contractAddress}'
      AND function = '${BCL_AFFILIATION_CONTRACT.FUNCTIONS.redeem_invitation_code}'
      AND data->'bcl-affiliation' IS NOT NULL
      AND jsonb_typeof(data->'bcl-affiliation'->'data'->'invitations') = 'array'
      AND data->'bcl-affiliation'->'data'->>'event_name' = 'InvitationRedeemed'
  `,
})
export class BclInvitationRedeemed {
  @PrimaryColumn()
  @ViewColumn()
  ...
  @ViewColumn()
  @ApiProperty({
    description: 'Event name (InvitationRedeemed)',
    required: false,
    nullable: true,
  })
  event_name?: string;
  ...
  @ViewColumn()
  @Index()
  @ApiProperty({ description: 'Redeemer address', required: false, nullable: true })
  redeemer?: string;
}
