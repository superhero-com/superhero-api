...
const { evidencePath } = require('../../../test/social-graph/evidence.cjs');
...
const enabled = process.env.SOCIAL_GRAPH_SCALE_TEST === 'true';
(enabled ? describe : describe.skip)(
  'Social graph million-row PostgreSQL qualification',
  () => {
    ...
    beforeAll(async () => {
      ...
      try {
        await new SocialGraphProjection1718900000035().up(runner);
      } finally {
    ...
    it('uses bounded index scans and constant-size deltas at one million incoming edges', async () => {
      await db.query(
        "INSERT INTO social_graph_projection_scopes(network,contract,generation,state) VALUES('ae_dev','ct_scale',1,'ready')",
      );
      const started = Date.now();
      await db.query(`INSERT INTO social_graph_projection_edges(network,contract,generation,from_address,to_address,kind)
      SELECT 'ae_dev','ct_scale',1,'ak_a'||translate(n::text,'0123456789','abcdefghij'),'ak_b','follow' FROM generate_series(1,1000000) n`);
      await db.query(
        "INSERT INTO social_graph_projection_counts(network,contract,generation,address,followers) VALUES('ae_dev','ct_scale',1,'ak_b',1000000)",
      );
      await db.query('ANALYZE social_graph_projection_edges');
      const seededMs = Date.now() - started;
      const explain =
        await db.query(`EXPLAIN (ANALYZE,BUFFERS,FORMAT JSON) SELECT id,from_address FROM social_graph_projection_edges
      ...
      const projection = new SocialGraphProjectionService(db),
        writesAt = Date.now();
      await Promise.all(
      ...
      writeFileSync(
        evidencePath('social-graph-million-postgres.json'),
        JSON.stringify(
          {
