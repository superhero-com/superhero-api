...
@Injectable()
export class SocialGraphCatchupService {
  constructor(
  ...
  async begin(
    ...
    await this.db.transaction(async (m) => {
      ...
      await m.query(
        `UPDATE social_graph_projection_scopes SET state=CASE WHEN synced_height IS NULL THEN 'catching-up' ELSE 'ready' END,sync_end_hash=$4,sync_end_height=$5,
  ...
  async applyPage(scope: ProjectionScope, page: CatchupPage): Promise<boolean> {
    ...
    return this.db.transaction(async (m) => {
        ...
        for (const event of tx.events) {
          ...
          for (const change of event.changes) {
            ...
            if (
              ...
              await m.query(
                `INSERT INTO social_graph_projection_outbox(network,contract,generation,tx_hash,event_index,height,follower,followed)
      ...
      if (page.nextCursor === null) {
        await m.query(
          `UPDATE social_graph_projection_scopes SET state='ready',synced_hash=sync_end_hash,synced_height=sync_end_height,
        ...
        await m.query(
          `UPDATE social_graph_projection_scopes SET sync_cursor=$4,sync_last_position=$5,sync_last_height=$6
          WHERE network=$1 AND contract=$2 AND generation=$3`,
          [
            ...key,
            page.nextCursor,
