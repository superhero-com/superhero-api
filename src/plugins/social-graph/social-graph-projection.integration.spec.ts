...
import { encodeGraphCursor } from './social-graph-reader';
...
(bin ? describe : describe.skip)(
  'Social graph projection on real PostgreSQL',
  () => {
    let pg: PostgresHandle,
      db: DataSource,
      service: SocialGraphProjectionService;
    ...
    beforeAll(async () => {
      ...
      try {
        await new SocialGraphProjection1718900000035().up(q);
      } finally {
        await q.release();
      }
      service = new SocialGraphProjectionService(db);
    }, 30000);
    afterAll(async () => {
      await db?.destroy();
      pg?.stop();
    ...
    beforeEach(async () => {
      for (const table of [
        ...
        await db.query(`TRUNCATE social_graph_projection_${table}`);
      await db.query(
        "INSERT INTO social_graph_projection_scopes(network,contract,generation,state) VALUES('ae_dev','ct_source',1,'ready'),('ae_dev','ct_destination',1,'ready')",
      );
    });
    it('deduplicates concurrent replay and counts only actual mutations', async () => {
      ...
      const count = await db.query(
        "SELECT followers,following FROM social_graph_projection_counts WHERE address='ak_b'",
      );
      ...
      expect(
        (
          await db.query(
            "SELECT followers FROM social_graph_projection_counts WHERE address='ak_b'",
          )
        )[0].followers,
      ).toBe('0');
    });
    it('keeps source and destination independent and preserves deletion activity', async () => {
      ...
      expect(
        (
          await db.query(
            "SELECT COUNT(*)::text AS n FROM social_graph_projection_edges WHERE contract='ct_destination'",
          )
        )[0].n,
      ).toBe('1');
    });
    it('rolls back all earlier mutations and replay identity when a batch fails', async () => {
      ...
      expect(
        (await db.query('SELECT * FROM social_graph_projection_edges')).length,
      ).toBe(0);
      expect(
        (await db.query('SELECT * FROM social_graph_projection_events')).length,
      ).toBe(0);
      expect(
        (await db.query('SELECT * FROM social_graph_projection_counts')).length,
      ).toBe(0);
    });
    ...
    it('resumes empty sparse snapshot pages atomically without producing follow events', async () => {
      ...
      const importer = new SocialGraphSnapshotService(db, service);
      await importer.begin(target, reader);
      expect(await importer.step(target, reader)).toBe(false);
      const resumed = new SocialGraphSnapshotService(db, service);
      expect(await resumed.step(target, reader)).toBe(true);
      const state = (
        await db.query(
          'SELECT state,export_cursor::text FROM social_graph_projection_scopes WHERE generation=2',
        )
      )[0];
      ...
      expect(
        (
          await db.query(
            "SELECT followers::text FROM social_graph_projection_counts WHERE generation=2 AND address='ak_b'",
          )
        )[0].followers,
      ).toBe('1');
      expect(
      ...
      expect(
        await db.query(
          'SELECT * FROM social_graph_projection_edges WHERE generation=2',
        ),
      ).toHaveLength(0);
    });
    it('does not advance snapshot checkpoints or retain edges on a malformed page or reorg', async () => {
      ...
      expect(
        (
          await db.query(
            'SELECT export_cursor::text FROM social_graph_projection_scopes WHERE generation=2',
          )
        )[0].export_cursor,
      ).toBe('0');
    ...
    it('serves scoped keyset pages and fails closed after generation cutover', async () => {
      ...
      await db.query(
        "INSERT INTO social_graph_projection_scopes(network,contract,generation,state) VALUES('ae_dev','ct_source',2,'importing')",
      );
      await expect(
    ...
    it('resumes ordered catch-up and exposes only completed key-block windows', async () => {
      await db.query(
        "UPDATE social_graph_projection_scopes SET snapshot_height=100,snapshot_hash='kh_start'",
      );
      const sync = new SocialGraphCatchupService(db, service);
      ...
      const resumed = new SocialGraphCatchupService(db, service);
      await expect(
      ...
      expect(
        (
          await db.query(
            "SELECT synced_height,synced_hash FROM social_graph_projection_scopes WHERE contract='ct_source'",
          )
        )[0],
      ...
      expect(
        (
          await db.query(
            'SELECT last_follow_height::text FROM social_graph_projection_rates',
          )
    ...
    it('rolls back an entire unordered sync page and rejects cross-page regression', async () => {
      await db.query(
        'UPDATE social_graph_projection_scopes SET snapshot_height=100',
      );
      const sync = new SocialGraphCatchupService(db, service);
      ...
      expect(
        await db.query('SELECT * FROM social_graph_projection_edges'),
      ).toHaveLength(0);
      expect(
        await db.query('SELECT * FROM social_graph_projection_events'),
      ).toHaveLength(0);
      ...
      expect(
        (
          await db.query(
            "SELECT sync_cursor FROM social_graph_projection_scopes WHERE contract='ct_source'",
          )
        )[0].sync_cursor,
      ).toBe('page2');
    });
    ...
    it('reconciles deleted accounts at the pinned watermark and fails closed on drift', async () => {
      ...
      await db.query(
        "UPDATE social_graph_projection_scopes SET synced_hash='kh_pinned',synced_height=101",
      );
      const reader: any = {
        identity: scope,
        assertCanonical: jest.fn().mockResolvedValue(undefined),
      ...
      const reconcile = new SocialGraphReconcileService(db, service);
      ...
      expect(await service.pending(scope)).toHaveLength(1);
      await expect(
        new SocialGraphQueryService(db).ready(scope.network, scope.contract),
      ).rejects.toThrow('not ready');
    });
    it('does not acknowledge reconciliation when activity changes during the chain read', async () => {
      ...
      expect(
        await new SocialGraphReconcileService(db, service).step(
          scope,
          reader,
          1,
        ),
    ...
    it('defers reconciliation while a live transaction prefix is being applied', async () => {
      await db.query(
        "UPDATE social_graph_projection_scopes SET synced_height=100,synced_hash='kh_start',sync_end_height=101,sync_end_hash='kh_end'",
      );
      ...
      expect(await service.pending(scope)).toHaveLength(2);
      expect(
        (await new SocialGraphQueryService(db).counts(scope, 'ak_b')).followers,
      ).toBe('1');
    });
    it('discards a connection after advisory unlock failure and permits subsequent worker passes', async () => {
      ...
      const worker = new SocialGraphWorkerService(
        db,
        {} as any,
        { isConfigured: () => true, getReader: () => reader } as any,
        {} as any,
        {} as any,
        {} as any,
        service,
      ...
      try {
          ...
          expect(
            (
              await probe.query(
                ...
                [`social-graph:${scope.network}:${scope.contract}`],
              )
            )[0].locked,
          ).toBe(true);
        } finally {
        ...
        if (previous === undefined)
          delete process.env.SOCIAL_GRAPH_WORKER_ENABLED;
    ...
    it('runs a restartable worker from snapshot to live generation, then rebuilds after reorg', async () => {
      ...
      process.env.SOCIAL_GRAPH_WORKER_ENABLED = 'true';
      try {
        await db.query('TRUNCATE social_graph_projection_scopes');
        const reader: any = {
          ...
          countsAt: jest.fn().mockResolvedValue({
            followers: '0',
            following: '0',
            blocked: '0',
          }),
        ...
        const make = () =>
          new SocialGraphWorkerService(
            db,
            { sdk: { getContext: () => ({ onNode: node }) } } as any,
            { isConfigured: () => true, getReader: () => reader } as any,
            new SocialGraphSnapshotService(db, service),
            new SocialGraphCatchupService(db, service),
            new SocialGraphReconcileService(db, service),
            service,
        ...
        expect(
          await db.query(
            'SELECT generation,state FROM social_graph_projection_scopes ORDER BY generation',
          ),
        ).toEqual([
    ...
    it('queues only actual live follows and retries scoped delivery without replay duplication', async () => {
      await db.query(
        'UPDATE social_graph_projection_scopes SET snapshot_height=100,notify_from_height=101',
      );
      const sync = new SocialGraphCatchupService(db, service);
      ...
      expect(
        await db.query('SELECT * FROM social_graph_projection_outbox'),
      ).toHaveLength(0);
      ...
      const outbox = new SocialGraphOutboxService(db, emitter);
      await outbox.dispatch(scope);
      expect(
        (
          await db.query('SELECT delivered FROM social_graph_projection_outbox')
        )[0].delivered,
      ).toBe(false);
      await outbox.dispatch(scope);
      expect(emitter.emitAsync).toHaveBeenCalledTimes(1);
      await db.query(
        "UPDATE social_graph_projection_outbox SET next_attempt_at=now()-interval '1 second'",
      );
      await outbox.dispatch(scope);
    ...
    it('does not dispatch outbox rows from an invalidated generation', async () => {
      await db.query(
        "INSERT INTO social_graph_projection_outbox(network,contract,generation,tx_hash,event_index,height,follower,followed) VALUES('ae_dev','ct_source',1,'th_old',0,100,'ak_a','ak_b')",
      );
      await service.invalidateAfterReorg(scope);
      const emitter: any = { emitAsync: jest.fn() };
      await new SocialGraphOutboxService(db, emitter).dispatch(scope);
    ...
    it('keeps live reads consistent while indexing complete transactions within a generation', async () => {
      await db.query(
        "UPDATE social_graph_projection_scopes SET synced_hash='kh_ready',synced_height=100,snapshot_height=90",
      );
      const sync = new SocialGraphCatchupService(db, service),
        queries = new SocialGraphQueryService(db);
    ...
    it('persists verified migration boundaries and never guesses from the projection snapshot', async () => {
      const target = { ...scope, generation: '2' },
        importer = new SocialGraphSnapshotService(db, service);
      const reader: any = {
        identity: scope,
      ...
      expect(
        await db.query(
          'SELECT * FROM social_graph_projection_scopes WHERE generation=2',
        ),
      ).toHaveLength(0);
      ...
      await importer.begin(target, reader);
      expect(
        await new SocialGraphQueryService(db).status(
          scope.network,
          scope.contract,
        ),
      ).toMatchObject({
    ...
    it('rolls back and reapplies its isolated schema migration cleanly', async () => {
      ...
      try {
        ...
        expect(
          (
            await runner.query(
              "SELECT to_regclass('social_graph_projection_scopes') AS name",
            )
          )[0].name,
        ).toBeNull();
        expect(
          (
            await runner.query(
              "SELECT to_regclass('social_graph_projection_outbox') AS name",
            )
          )[0].name,
        ).toBeNull();
        ...
        ).toBe('social_graph_projection_scopes');
      } finally {
