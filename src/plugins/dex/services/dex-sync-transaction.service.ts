...
@Injectable()
export class DexSyncTransactionService extends BasePluginSyncService {
  protected readonly logger = new Logger(DexSyncTransactionService.name);
  ...
  private async saveTransaction(tx: Tx): Promise<PairTransaction | null> {
    if (tx.contract_id !== DEX_CONTRACTS.router) {
      return null;
    }
  ...
  private async extractPairInfoFromTransaction(tx: Tx) {
