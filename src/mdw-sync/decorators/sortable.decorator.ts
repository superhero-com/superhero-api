 ...
 * hash: string;
 */
export const Sortable = (options?: SortableOptions | string): PropertyDecorator => {
  ...
  return (target: any, propertyKey: string | symbol) => {
    ...
    Reflect.defineMetadata(SORTABLE_METADATA_KEY, { description }, target, propertyKey);
    
    // Also store the field name and description in a class-level map for easy retrieval
    const existingFields = Reflect.getMetadata(SORTABLE_FIELDS_KEY, target) || new Map();
