...
import { createEntityConfigRegistry, EntityConfigRegistry } from '../utils/relation-loader';
...
export function createEntityControllers<T extends EntityConfig<any>>(
  ...
  return configs.map((config) => createEntityController(config, configRegistry));
}
