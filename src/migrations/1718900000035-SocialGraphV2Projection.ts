...
export class SocialGraphV2Projection1718900000035 implements MigrationInterface {
  async up(q: QueryRunner): Promise<void> {
    await q.query(`CREATE TABLE social_graph_v2_scopes (
    ...
    await q.query(`CREATE TABLE social_graph_v2_edges (
    ...
    await q.query(
      'CREATE INDEX social_graph_v2_followers ON social_graph_v2_edges(network,contract,generation,to_address,kind,id DESC)',
    );
    await q.query(
      'CREATE INDEX social_graph_v2_following ON social_graph_v2_edges(network,contract,generation,from_address,kind,id DESC)',
    );
    await q.query(`CREATE TABLE social_graph_v2_counts (
    ...
    await q.query(`CREATE TABLE social_graph_v2_dirty (
      network text NOT NULL, contract text NOT NULL, generation bigint NOT NULL, address text NOT NULL,
      revision bigint NOT NULL DEFAULT 1, PRIMARY KEY(network,contract,generation,address))`);
    await q.query(`CREATE TABLE social_graph_v2_events (
    ...
    await q.query(`CREATE TABLE social_graph_v2_outbox (
      network text NOT NULL, contract text NOT NULL, generation bigint NOT NULL,
      tx_hash text NOT NULL, event_index integer NOT NULL, height bigint NOT NULL,
  ...
  async down(q: QueryRunner): Promise<void> {
    for (const table of [
      ...
      await q.query(`DROP TABLE social_graph_v2_${table}`);
    }
  }
}
